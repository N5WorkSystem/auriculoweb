...
if not os.path.exists('auriculoterapia.db'):
    criar_banco_de_dados()  # type: ignore # Função que cria e inicializa o banco
...
#---------------------------------------------------------------------
# Formulário de Entrada e Diagnóstico
#---------------------------------------------------------------------
def diagnosticar():
    conn = sqlite3.connect('auriculoterapia.db')
    cursor = conn.cursor()
...
#--------------------------------------------------------
# Salvamento dos dados no diagnóstico
#--------------------------------------------------------
@app.route('/diagnostico', methods=['POST'])
def diagnostico():
    ...
    # ...
    protocolo_formatado = "; ".join([f"{item['ponto']} - {item['funcao']}" for item in protocols])
    ...
    return render_template('resultado.html', protocolo=protocols)
